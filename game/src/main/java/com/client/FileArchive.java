...
public class FileArchive {
	...
	public byte[] readFile(String s) {
		return getArchiveData(s);
	}

	public byte[] getArchiveData(String s) {
		if (Configuration.loadExternalCacheArchives && new File(Signlink.getCacheDirectory() + Configuration.EXTERNAL_CACHE_ARCHIVE + s).exists()) {
			try {
				//System.out.println("Loaded external file: " + s);
				return getBytesFromFile(new File(Signlink.getCacheDirectory() + Configuration.EXTERNAL_CACHE_ARCHIVE + s));
			} catch (IOException e) {
				e.printStackTrace();
