...
public class RSFont extends Rasterizer2D {
	...
	public static String startImageCustom;
				...
				if (character == 60) {
					...
					if (character == 62 && startIndex != -1) {
						...
						if (effectString.equals(startEffect)) {
							...
							} else if (effectString.startsWith(startImageCustom)) {
									try {
					...
					if (effectString.equals(startEffect)) {
						...
						if (effectString.startsWith(startImage)) {
							...
							try {
								...
								Sprite icon = ImageCache.get(iconId);
								int iconOffsetY = icon.maxHeight - 4;
								if (transparency == 256) {
			...
			if (character == 60) {
				...
				if (character == 62 && startIndex != -1) {
					...
					if (effectString.equals(startEffect)) {
						...
						if (effectString.startsWith(startImageCustom)) {
							try {// <img=
								int iconId = Integer.valueOf(effectString.substring(5));
								finalWidth += ImageCache.get(iconId).maxWidth;
							} catch (Exception exception) {
								exception.printStackTrace();
							}
						}
